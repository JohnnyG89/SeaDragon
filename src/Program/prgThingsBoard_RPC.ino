...
//Note - I haven't yet figured out how to make this compatible with the prgMQTT task.
//However, it has been tested and works.
//Use it as an either/or with prgMQTT for now.

static        Logging       tbrpcLog;
static EthernetClient  ethclient_tb2;
static ThingsBoard     tb2(ethclient_tb2);

//Standard connection credentials.
static const  char*         TOKEN                     = "VqSbL0ENWMvPUA7GTLBd";
static const  char*         THINGSBOARD_SERVER        = "192.168.1.200";

static const  size_t        	callbacks_size            = 27;
...
void initThingsBoard_RPC(void) {
  ...
  if (!tb2.connected()) {
    ...
    if (!tb2.connect(THINGSBOARD_SERVER, TOKEN)) {
  ...
  if (!tb2.RPC_Subscribe(callbacks, callbacks_size)) {
...
static RPC_Response rpcCheckStatus(const RPC_Data &data) {
  ...
  //return RPC_Response("checkStatus", iMachineModeSwitchWord[0]);
}
